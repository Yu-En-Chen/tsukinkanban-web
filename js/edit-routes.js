...
export function startRouteEditMode(cardId, currentLineIds) {
    ...
    // 2. 把實心玻璃面板往上拉
    extensionCard.style.transition = 'transform 0.55s cubic-bezier(0.2, 0.8, 0.2, 1)';
    ...
    scrollWrapper.style.height = `calc(100dvh - env(safe-area-inset-top, 0px) - 32px)`;
    ...
    editContainer.style.cssText = 'display: flex; flex-direction: column; gap: 8px; flex: 1; padding-bottom: 24px; opacity: 0; transition: opacity 0.3s ease 0.15s;';
    ...
    btnContainer.style.transition = 'opacity 0.3s ease 0.15s';
    ...
    const createBtn = (iconHtml, text, isPrimary, onClick) => {
        ...
        if (isPrimary) btn.style.background = 'rgba(255, 255, 255, 0.15)'; 
