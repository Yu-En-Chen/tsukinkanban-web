// personalization.js - 卡片個人化 (Customize) 彈窗
...
(function injectGlobalNavigationBlocker() {
    ...
    document.addEventListener('click', (e) => {
        ...
        if (window.pSyncing) {
            ...
            if (protectedBtn) {
                e.preventDefault();
                e.stopPropagation(); // 在事件抵達原本的 onclick 前攔截
    ...
    }, { capture: true }); // capture 階段攔截，最先收到點擊事件
    ...
    window.addEventListener('keydown', (e) => {
        ...
        // ArrowLeft：關閉個人化卡片 (Customize)
        if (e.key === 'ArrowLeft') {
            const blankOverlay = document.getElementById('dynamic-blank-overlay');
            if (blankOverlay && typeof window.closeBlankOverlay === 'function') {
                e.preventDefault(); 
                window.closeBlankOverlay(true); // true：以手勢動畫關閉
                return;
            }
        }

        // ArrowDown：關閉詳情卡片 (Detail Card)
        if (e.key === 'ArrowDown') {
            // 個人化卡片開啟中則不處理，避免一次關掉兩層
            const blankOverlay = document.getElementById('dynamic-blank-overlay');
            if (blankOverlay) return;

            // 以 card-stack 的 has-active 判斷詳情卡片是否展開
...
<div class="card-content" id="p-ghost-wrapper" style="position: relative;">
    ...
    <div id="p-edit-row" style="--btn-height: 44px; display: flex; gap: 8px; position: relative;
        ...
        <button id="p-btn-circle-1" class="info-tag-item interactive-btn" aria-label="表示名をコピー" onclick="window.handleCopyAction(event, 'name', this)" style="
        ...
        <button id="p-btn-circle-2" class="info-tag-item interactive-btn" aria-label="表示名を貼り付け" onclick="window.handlePasteAction(event, 'name', this)" style="
    ...
    <div id="p-color-edit-row" style="--btn-height: 44px; display: flex; gap: 8px; position: relative;
        ...
        <button id="p-btn-color-circle-1" class="info-tag-item interactive-btn" aria-label="カラーをコピー" onclick="window.handleCopyAction(event, 'color', this)" style="
            cursor: pointer; height: var(--btn-height); width: var(--btn-height); padding: 0; border-radius: 50%; position: relative; overflow: hidden; display: block; flex-shrink: 0; transition: transform 0.4s var(--apple-spring), box-shadow 0.4s var(--apple-spring);">
        ...
        <button id="p-btn-color-circle-2" class="info-tag-item interactive-btn" aria-label="カラーを貼り付け" onclick="window.handlePasteAction(event, 'color', this)" style="
            cursor: pointer; height: var(--btn-height); width: var(--btn-height); padding: 0; border-radius: 50%; position: relative; overflow: hidden; display: block; flex-shrink: 0; transition: transform 0.4s var(--apple-spring), max-width 0.4s var(--apple-spring), margin 0.4s var(--apple-spring), padding 0.4s var(--apple-spring); max-width: var(--btn-height);">
...
`;
        ...
        let wheelGestureLocked = false; // 阻斷觸控板滑動慣性的鎖

        overlay.addEventListener('wheel', (e) => {
            // 翻轉中、已觸發關閉或已上鎖時，忽略殘留的慣性事件
            if (window.pSyncing || window.isFlipAnimating || wheelGestureLocked) return;

            // 1. 偵測橫向滑動
            if (Math.abs(e.deltaX) > Math.abs(e.deltaY)) {
                e.preventDefault(); // 攔截瀏覽器的上一頁/下一頁手勢
                ...
                if (wheelDeltaX > 0) wheelDeltaX = 0; // 僅允許向左滑關閉
                ...
                // 不等慣性停止：progress > 0.2 即觸發關閉
                if (progress > 0.2 || wheelDeltaX < -100) {
                    wheelGestureLocked = true; // 此卡片關閉前不再接受 wheel
    ...
    // =========================================================
    // 個人化面板：與復原 (Undo) 按鈕連動的輸入框動畫
    // =========================================================
    window.startInputUndoAnimation = function() {
        const nameEls = getElements('name');
        const colorEls = getElements('color');
        
        // 輸入框不存在或未顯示（未開啟個人化卡片）時跳過
    ...
    window.finishInputUndoAnimation = function(isSuccess) {
        ...
        // 階段二：依結果將 Sync 圖示換成打勾或打叉
        [window.activeUndoSvgs.name, window.activeUndoSvgs.color].forEach(item => {
            if (!item) return;
            const { syncIcon, checkIcon, errorIcon } = item;
            
            // Sync 圖示往下掉出畫面（維持旋轉角度避免回彈）
    ...
    window.resetInputUndoAnimation = function(immediate = false) {
        ...
        [window.activeUndoSvgs.name, window.activeUndoSvgs.color].forEach(item => {
            ...
            if (!immediate) {
                ...
                els.display.style.transition = 'none';
                els.display.style.transform = 'translate3d(0, 40px, 0)'; 
                void els.display.offsetWidth; // 強制 reflow
...
window.toggleGhostEditMode = function(type, e, element) {
    ...
    // 不做 appendChild 遷移，改用 RAF 逐幀追蹤（避免 Android 收起鍵盤）
    // 1. 輸入框固定在 wrapper 根部，不跨容器移動 DOM（Android 會因此收起鍵盤）
    ...
    // 3. 60fps 追蹤迴圈：隱形輸入框跟隨 Flexbox 動畫縮放與平移
    const startTime = Date.now();
    const trackAnim = () => {
        if (window.pActiveEditType !== type) return; // 使用者已切換到其他輸入框
        ...
        // 追蹤 500ms（涵蓋 400ms 的 spring 動畫加緩衝）
